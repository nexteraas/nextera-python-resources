...
class Boltz1Predictor(object):
    ...
    def _move_out_files(self, out_fn):
        b=os.path.exists(DockerInterop._TMP_DIR)
        print('DockerInterop._TMP_DIR:' + str(b))
        b = os.path.exists(DockerInterop._TMP_DIR + 'boltz_results_' + self._in_fn + '/predictions')
        print('DockerInterop._TMP_DIR + boltz_results_ + self._in_fn + /predictions:' + str(b))
        b = os.path.exists(DockerInterop._TMP_DIR + 'boltz_results_' + self._in_fn + '/predictions/' + self._in_fn)
        print('.,:' + str(b))
